...
### 2.1 当前补充修复（截至 2026-03-31）

近期已补齐以下问题，后续实现与回归测试需按该状态对齐：

- 前端关键页面源码与文案已统一为 UTF-8，修复部署后首页和课堂页的中文乱码
- 设置同步逻辑已补充“可用 provider 存在但 `modelId` 为空时自动选择默认模型”，避免误报“请选择一个模型开始使用”
- `课堂操作` 已并入右侧栏，与 `笔记`、`对话` 作为同级切换，避免遮挡左侧幻灯片导航
- `重制当前页` 已补齐“服务端真值前置保存”与“左侧导航当前页联动显示”
- 重制应用后已额外保存 `Applied regeneration job <jobId>` 系统版本，支持恢复旧版本后再切回重制内容
- 重制链路已从单一对话模型扩展为“LLM + 图片/视频 + TTS”的已配置可用模型协同执行
- 已修复 `lib/server/course-regeneration.ts` 中阻塞 Docker 构建的 TypeScript 类型问题
- 已将重制阶段的图片、视频、TTS 生成统一到与常规生成一致的服务端 helper、参数归一化与日志格式
- 已收敛 KaTeX `newLineInDisplayMode` 兼容性告警，避免课程生成与白板公式渲染阶段持续刷屏
...
课堂页保留原有播放讲解界面，同时新增显式“编辑当前页”模式切换。

核心原则：

- 原有播放讲解能力不删除、不改主链路
- 手动编辑不直接发生在播放态主画布上
- 用户必须先显式进入 `edit` 模式，才能操作当前页
...
- 离开页面前提示未保存变更
- 切换到课堂操作页签后不遮挡左侧页面导航
- 左侧导航的当前选中页要作为课堂操作中的当前作用页面
- 进入编辑态时若存在播放会话，先停止播放再进入编辑
- 退出编辑态时不自动恢复播放，由用户手动重新开始讲解

建议组件调整：

- `Stage` 继续负责播放讲解态
- 新增 `SlideEditorCanvas` 负责编辑态中间画布
- 新增 `EditModeBanner` 显示编辑中状态和返回播放入口
- `ClassroomOpsPanel` 增加 `编辑当前页 / 返回播放` 按钮与编辑态状态提示

### 4.4.3 对话重制模式

课堂页在 `课堂操作` 页签中提供 `Rework with Prompt` 面板。
...
- 先生成到草稿态，再由用户确认应用
- 支持保留人工修改内容
- 在 `笔记 / 对话 / 课堂操作` 间切换时，不影响左侧幻灯片导航浏览
- `重制当前页` 的请求目标必须与左侧当前选中页一致
- 若服务端课堂不存在，不能直接向用户暴露 `Classroom not found`，而应先尝试自动保存
- 长时任务等待过程中，前端必须显示加载动画、进度条与后端阶段文案，而不是只显示静态等待

建议前端新增辅助函数：

- `ensureClassroomPersistedBeforeRegenerate(classroomId, stage, scenes)`
  - 先尝试 `GET /api/classroom/:id`
  - `404` 时执行 `PATCH /api/classroom/:id`
  - 成功后再进入重制创建逻辑
...
preparing
  前端：显示“正在准备重制任务”
  后端：读取 classroom / stage / scenes / context

resolving-model
  前端：显示当前模型解析中
  后端：选择已配置可用的 LLM provider/model
...
generating-media
  前端：显示媒体资源生成中
  后端：调用与常规生成一致的图片/视频服务端 helper，先完成参数归一化，再生成并回填资源 URL

generating-tts
  前端：显示讲稿音频生成中
  后端：调用与常规生成一致的 TTS 服务端 helper，按真实返回格式落盘并回填音频 URL

assembling-preview
  前端：显示预览整理中
  后端：拼装 preview、changedSceneIds、message
...
- 本地草稿未落盘时发起重制
- 左侧切换页面后当前作用页面同步
- 长时间等待时阶段动画与后端 step 同步
- 预览结果
- 应用结果
- 丢弃结果
- 恢复旧版本后重新切回某次已应用重制结果
- 包含图片、视频或讲稿音频的页面重制
- 重制阶段的图片日志、视频日志、TTS 日志字段与常规生成链路一致
- 含 `\\` 或 `\newline` 的公式页面重制、白板公式动作执行后，服务端日志不再反复输出 `newLineInDisplayMode`
...
- 直接编辑可保存和恢复
- 课堂在播放态下仍保留原有讲解入口
- 用户可通过 `课堂操作 -> 编辑当前页` 进入编辑态，并在编辑后返回播放态
- 重制不会静默覆盖原课程
- 重制结果可应用、可丢弃、可回退
- 首页与课堂页关键中文文案无乱码
- 已配置可用模型时，点击“进入课堂”不会再误报未选择模型
- 切换到课堂操作页签后，左侧幻灯片导航仍可正常使用
- 课堂仅存在本地草稿时，点击“重制当前页”不会直接报 `Classroom not found`
- 左侧切换页面后，课堂操作中的当前作用页面会同步变化，且重制结果作用于该页
- 恢复历史版本后，仍可切换回某次已应用的重制结果
- 页面重制若涉及图片、视频、讲稿音频，预览中能看到相应媒体资源
- 重制阶段的图片、视频、TTS 日志字段与常规生成保持一致，便于联调排障
- 含 `\\` 或 `\newline` 的公式渲染不会在服务端日志中持续刷 `newLineInDisplayMode`
...
6. 应用重制结果
7. 恢复到上一个历史版本
8. 再恢复到 `Applied regeneration job ...` 版本
9. 对包含图片、视频或讲稿音频的页面执行重制，确认预览资源可生成
10. 导出 PPT 与资源包
